...
class DescendantsByCategoryView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        descendant_categories = Category.objects.filter(id__in=descendant_ids)
        descendant_products = Product.objects.filter(category_id__in=descendant_ids)
...
class ParentsByCategoryView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    permission_required = 'django_db_app.view_category'
    raise_exception = True
    ...
    def form_valid(self, form):
        ...
        return render(self.request, self.template_name, {
            'form': form,
            'selected_category': category,
            'parent_categories': parent_chain[::-1],  # чтобы шли от корня к выбранной
        })


class TerminalCategoriesView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    permission_required = 'django_db_app.view_category'
    raise_exception = True
    ...
    def form_valid(self, form):
        ...
        all_categories = list(Category.objects.all().values('id', 'parent_id', 'name', 'measure_id'))
        ...
        def get_terminal_categories(category_id, depth=0):
            ...
            if not children:
                current = next(c for c in all_categories if c['id'] == category_id)
                terminals.append({
                    'id': current['id'],
                    'name': current['name'],
                ...
                for child in children:
                    terminals.extend(get_terminal_categories(child['id'], depth + 1))
            return terminals
...
class ProductsWithParamsView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        products = Product.objects.filter(category_id__in=category_ids).select_related('category', 'category__measure')

        results = []

        for product in products:
            # Сначала получаем параметры самого продукта
            param_values = ParameterValue.objects.filter(product=product).select_related('param', 'param__measure',
                                                                                         'value_enum')

            # Затем параметры категории, если не переопределены на уровне продукта
            ...
            for pv in combined_params:
                ...
                if param.data_type == 'int':
                    ...
                    value = pv.value_enum.value_str or pv.value_enum.value_int or pv.value_enum.value_real or pv.value_enum.value_path
                ...
                results.append({
                    ...
                    'measure': product.category.measure.name_short if product.category.measure else '',
                    'price': product.price,
                    'param_id': param.id,
                    'param_name': param.name_short,
                    'param_type': param.data_type,
                    'param_value': value,
                    'param_measure': param.measure.name_short if param.measure else ''
                })
...
class AllProductsWithParamsView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        products = Product.objects.filter(category=root_category).select_related('category', 'category__measure')
        ...
        for product in products:
            ...
            # Параметры, заданные для категории, если они не переопределены на уровне продукта
            overridden_params = {pv.param_id for pv in param_values}
            inherited_param_values = ParameterValue.objects.filter(category=product.category).exclude(
                param_id__in=overridden_params).select_related('param', 'param__measure', 'value_enum')
            ...
            for pv in combined_params:
                ...
                if param.data_type == 'int':
                    value = str(pv.value_int) if pv.value_int is not None else ''
                elif param.data_type == 'real':
                    value = str(pv.value_real) if pv.value_real is not None else ''
                elif param.data_type == 'str':
                    value = pv.value_str if pv.value_str is not None else ''
                elif param.data_type == 'path':
                    value = pv.value_path if pv.value_path is not None else ''
                elif param.data_type == 'enum' and pv.value_enum:
                    value = pv.value_enum.value_str or pv.value_enum.value_int or pv.value_enum.value_real or pv.value_enum.value_path
                else:
                ...
                try:
                    ...
                    print(f"Error occurred while processing product {product.name}: {e}")
                    continue
...
class ProductParamsView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    permission_required = 'django_db_app.view_product'
    ...
    def get_context_data(self, **kwargs):
        ...
        if form.is_valid():
            ...
            params = ParameterValue.objects.filter(product=product).select_related('param', 'param__measure',
                                                                                   'value_enum')
        ...
        if product and not params:
            context['no_params_message'] = "Параметры для выбранного продукта не найдены."
...
class ProductsWithAggregateParamsView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        aggregate_params = self.filter_params_by_aggregate(products_with_params, parent_param_id)
    ...
    def get_products_with_params(self):
        ...
        # Получаем все продукты с оптимизированными запросами для связанных объектов
        products = Product.objects.select_related('category', 'category__measure').all()
        ...
        for product in products:
            ...
            # Получаем наследуемые параметры категории, если они не переопределены на уровне продукта
            inherited_param_values = ParameterValue.objects.filter(
                category=product.category
            ).exclude(param_id__in=used_param_ids).select_related(
                'param', 'param__measure', 'value_enum'
            )
    ...
    def filter_params_by_aggregate(self, products_with_params, parent_param_id):
        ...
        parent_params = ParameterAggregate.objects.filter(parent_param_id=parent_param_id)
