    ...
    def clean(self):
        ...
        if is_enum and parent and parent.is_enum:
            raise forms.ValidationError(
                "Категория-перечисление не может быть подкатегорией другой категории-перечисления.")
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form.base_fields['parent'].queryset = Category.objects.filter(is_enum=False)
        return form
...
class EnumValueForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if category and not category.is_enum:
            raise forms.ValidationError("Можно добавлять значения только для категорий-перечислений.")
...
class EnumValueAdmin(admin.ModelAdmin):
    ...
    def get_value(self, obj):
        return obj.value_str or obj.value_int or obj.value_real or obj.value_path
...
class ParameterForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if data_type == 'enum' and not enum:
            raise forms.ValidationError("Для типа 'enum' необходимо указать категорию-перечисление.")
        if data_type != 'enum' and enum:
            raise forms.ValidationError("Категория-перечисление может быть указана только для типа 'enum'.")
...
class ParameterAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        form = super().get_form(request, obj, **kwargs)
        form.base_fields['enum'].queryset = Category.objects.filter(is_enum=True)
        return form
...
class ParameterValueForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if param and param.data_type == 'enum' and not value_enum:
            raise forms.ValidationError("Для параметра типа 'enum' необходимо указать значение перечисления.")
...
class ParameterValueAdmin(admin.ModelAdmin):
    ...
    def get_value(self, obj):
        return obj.value_str or obj.value_int or obj.value_real or obj.value_path or (
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if obj and obj.param and obj.param.data_type == 'enum':
            form.base_fields['value_enum'].queryset = EnumValue.objects.filter(category=obj.param.enum)
        return form
...
admin.site.register(ParameterAggregate, ParameterAggregateAdmin)