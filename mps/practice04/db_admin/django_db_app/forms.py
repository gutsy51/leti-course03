...
class CategorySelectForm(forms.Form):
    category = forms.ModelChoiceField(
        ...
        empty_label="Не выбрано"  # Это добавляет опцию "Не выбрано" в выпадающий список
    )
...
class ParentParamForm(forms.Form):
    parent_param_id = forms.IntegerField(
        ...
        required=True,
        widget=forms.NumberInput(attrs={'placeholder': 'Введите идентификатор параметра'})
    )

    def clean_parent_param_id(self):
        parent_param_id = self.cleaned_data.get('parent_param_id')

        # Можно добавить дополнительные проверки, например, на существование такого параметра в базе
        if not Parameter.objects.filter(id=parent_param_id).exists():
            raise forms.ValidationError("Родительский параметр с таким ID не существует.")
